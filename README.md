...
1. **Database Schema (`warehouse.db`)**:
   ...
   - **Private Lockers (Zone P)**: Dedicated racks P1 (Pinky's Private Locker) and P2 (Vinod's Private Locker) for personal items.
2. **Flask Backend (`app.py`)**:
   ...
   - **Auto-Scan Safeguard**: Excludes Zone P lockers from the auto-assignment query (`AND zone != 'P'`) so personal lockers are never overwritten by standard barcode scans.
...
4. **Enterprise UI Dashboard**:
   ...
   - **Zone P Display Integration**: Adds a "Private Lockers" tab to view Zone P. Styled with purple glassmorphism and a private status indicator. Shows designated labels ("Pinky's Private Locker" / "Vinod's Private Locker").
...
The dashboard loads with all 30 racks vacant and the 2 private lockers in the designated "Private" state. Clicking the "Coca-Cola" scan simulator button instantly triggers the database assignment. 
...
### 2. Private Lockers Integration (Zone P)
- The navigation tabs now include an interactive **Private Lockers** tab.
- When clicked, it renders Zone P, displaying the two purple lockers marked as `PRIVATE` with custom labels:
  - **P1**: Pinky's Private Locker
  - **P2**: Vinod's Private Locker
- The auto-scan logic skips Zone P empty slots, keeping them safe from accidental scans.

### 3. Device Web Camera Barcode Scanner Modal
